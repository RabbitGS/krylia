...
(function (global) {
  ...
  Widget.prototype.renderGrid = function () {
    var self = this, d = this.data;
    var all = this.filtered(true, true);   // совпавшие с фильтрами — для счётчика и подсветки
    var matchIds = {};
    all.forEach(function (x) { matchIds[x.id] = 1; });
    var res = this.root.querySelector('.shm__results');
    ...
    houses.forEach(function (house) {
      // каркас сетки — из ВСЕХ квартир подъезда: фильтр не сужает столбцы и этажи,
      // несовпавшие квартиры приглушаются (is-dimmed), размер ячеек стабилен
      var flats = self.flatsOf(house);
    ...
    var noMatch = all.length ? '' : '<div class="shm__empty">Под фильтры ничего не подошло.</div>';
    var legend = noMatch + '<div class="shm__legend">' + ['free', 'reserved', 'promo', 'sold'].map(function (k) {
